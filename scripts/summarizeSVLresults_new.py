...
def parsePEResultsFromFile(url,verbose=False, doPlots=False):
	...
	for f in fileNames:
		...
		selection = ''
		try:
			syst1, syst2, massstr,  _ = tag.rsplit('_', 3)
			syst = syst1+'_'+syst2
		except ValueError:
...
#		useForCalib=True if 'nominal' in syst else False
		useForCalib=True if 'mass' in syst else False
		for key in fIn.GetListOfKeys():
			if verbose:
				print '  %-18s  ' % key.GetName(),

			keyName=key.GetName()
			PEsummary=analyzePEresults(key=keyName,fIn=fIn,outDir=url,doPlots=useForCalib,syst=(syst+massstr))
			if not 'bias' in PEsummary : continue

			bias, biasErr = PEsummary['bias']
			if verbose:
				print massstr+': %6.3f +- %5.3f' % (bias, biasErr)
	...
	for key,grColl in sorted(grCollMap.items()):
		...
		for tag,gr in sorted(grColl.items()):
			...
			if doFit and (gr.Integral()!=0):
				...
				slope=(gr.GetFunction('pol1')).GetParameter('par2')#(1)
...
def main():
	...
	if opt.calib:
		results,calibGrMap,resCalibGrMap = parsePEResultsFromFile(url=opt.calib, verbose=True)
